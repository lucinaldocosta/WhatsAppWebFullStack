...
import "../Styles/Components/GetMessages.css";

function GetMessages(props) {
  const loggedData = JSON.parse(sessionStorage.getItem("logged"));

  // Estado para armazenar o nome do amigo selecionado.
  const [friendName, setFriendName] = useState("");

  // Estado para armazenar os dados das mensagens a serem obtidas.
  ...
  // Estado para armazenar todas as mensagens obtidas
  const [getAllMessages, setGetAllMessages] = useState([]);

  // Função para obter as mensagens do backend.
  const getMessagesBackend = () => {
    if (getMessages.WhoReceive !== "") {
      fetch("http://localhost:3000", {
        ...
        .then((data) => {
          ...
          setGetAllMessages(() => allMessages);
        });
    }
  };

  // Efeito que atualiza o nome do amigo selecionado quando ocorre uma mudança.
  useEffect(() => {
    props.onFriendSelect(friendName);

    const handleClick = () => {
      const loggedData = JSON.parse(sessionStorage.getItem("logged"));
      setFriendName(loggedData.messageFor);
      ...
      setTimeout(() => {
        const element = document.getElementById("article_messages");
        function downScroll() {
          element.scrollTo({
            top: elementScroll,
            behavior: "instant",
  ...
  }, [loggedData.messageFor]);

  // Efeito que obtém as mensagens do backend em intervalos regulares.
  useEffect(() => {
    getMessagesBackend();
    const interval = setInterval(getMessagesBackend, 1000);
    return () => {
      clearInterval(interval);
    };
  }, [getMessages.WhoReceive]);

  // Efeito que obtém as mensagens do backend quando há uma atualização de mensagens.
  useEffect(() => {
  ...
  return (
    <>
      {/* Seção para exibir a foto e descrição do whatsapp web */}
      <section
        className={friendName !== "" ? "section_photo_off" : "section_photo"}
      >
        {/* Elemento para exibir a foto */}
        <article className="article_photo">
          <svg
            viewBox="0 0 303 172"
            width="360"
        ...
        <article className="article_photo_description">
          <h1>Whatsapp Web</h1>
          <p>
            Sends and receives messages without the need to have your phone
            connected
          </p>
        </article>
      </section>

      {/* Seção para exibir o nome do amigo selecionado */}
      <section className="section_head_friend_name">
        <article className="article_head_friend_name">
          {/* Titulo para exibir o nome do amigo */}
          <h3 className={friendName === "" ? "" : "title_head_friend_name"}>
            {friendName}
          </h3>
        </article>
      </section>
      ...
      <section
        className={
          friendName === "" ? "section_messages_off" : "section_messages"
        }
      >
        {/* Elemento para exibir as mensagens */}
        <article className="article_messages" id="article_messages">
          {/* Mapeia todas as mensagens e renderiza os elementos de acordo */}
          {getAllMessages.map((item, index) => {
            if (item.WhoSend === loggedData.userLogged) {
              // Renderiza a mensagem enviada pelo usuário logado.
              return (
                <p className="p_own_messages" key={index}>
                  {item.Message}
                </p>
              );
            } else {
              // Renderiza a mensagem enviada pelo amigo.
              return (
                <p className="p_friend_messages" key={index}>
                  {item.Message}
                </p>
              );
            }
